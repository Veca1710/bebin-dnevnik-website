...
const colors = { primary:"#9FAE8E", primaryDark:"#6FA258", background:"#FAFAF8", cream:"#F7F3EC", sageLight:"#EEF1E8", border:"#E7E8E2", text:"#40443C", body:"#5A5E55", muted:"#94988E" };
...
const features = [
  [Milk, "Dojenje, izmlazanje i dohrana", "Pratite trajanje dojenja, vreme hranjenja i količinu dohrane na jednom mestu."],
  [Moon, "Spavanje", "Zabeležite kada je beba spavala i lakše pratite dnevni ritam."],
  [Droplets, "Pelene i rutina", "Jednostavan unos svakodnevnih aktivnosti koje roditelji najčešće prate."],
  [Clock, "Pregled dana", "Sve aktivnosti su organizovane po danu, tako da brzo vidite šta se dešavalo."],
  [Smartphone, "Radi kao aplikacija", "Možete je dodati na početni ekran telefona i koristiti kao običnu aplikaciju."],
  [Printer, "Pregled za deljenje", "Korisno kada želite da pokažete ritam bebe partneru, patronažnoj sestri ili pedijatru."]
];
      ...
      <section className="relative overflow-hidden px-5 py-20 md:px-8 md:py-28">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:.6}}>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Button className="h-14 rounded-full px-8 text-base text-white shadow-[0_16px_36px_rgba(95,138,82,0.28)]" style={{backgroundColor:colors.primaryDark}}><HeartHandshake size={19}/>Podrži projekat</Button><a href="https://timov-dnevnik.vercel.app/" target="_blank" rel="noopener noreferrer" className="inline-flex h-14 items-center justify-center rounded-full border bg-white/70 px-8 text-base font-medium backdrop-blur" style={{borderColor:colors.primary,color:"#173B2F"}}>Otvori aplikaciju</a></div>
          </motion.div>
  ...
  return <div className="min-h-screen scroll-smooth" style={{backgroundColor:colors.background,color:colors.text}}>
    ...
    <main>
      <section id="pocetna" className="relative min-h-screen overflow-hidden px-4 pb-12 pt-28 md:px-8 md:pb-20 md:pt-32">
        ...
        <div className="absolute inset-0 bg-cover bg-center md:hidden" style={{backgroundImage:"linear-gradient(180deg, rgba(250,250,248,0.34) 0%, rgba(250,250,248,0.78) 43%, rgba(250,250,248,0.96) 76%, rgba(250,250,248,1) 100%), url('/background-mobile.png')",backgroundPosition:"center top"}}/>
        ...
        <div className="relative mx-auto grid min-h-[720px] max-w-7xl items-center gap-8 md:min-h-[760px] lg:grid-cols-[1.05fr_0.95fr]">
          <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:.6}} className="mx-auto max-w-2xl pt-[28vh] text-center sm:pt-[32vh] md:pt-0 lg:ml-[38%] lg:text-left"assName="mx-auto max-w-2xl pt-[42vh] text-center sm:pt-[48vh] md:pt-0 lg:ml-[38%] lg:text-left">
            ...
            <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start"><a href="https://timov-dnevnik.vercel.app/" target="_blank" rel="noopener noreferrer"><Button className="h-14 w-full rounded-full px-8 text-base text-white shadow-[0_16px_36px_rgba(95,138,82,0.32)] sm:w-auto md:h-16 md:px-9 md:text-lg" style={{backgroundColor:colors.primaryDark}}><HeartHandshake size={21}/>Otvori aplikaciju</Button></a><a href="#kako-radi"><Button variant="outline" className="h-14 w-full rounded-full bg-white/55 px-8 text-base backdrop-blur-md sm:w-auto md:h-16 md:px-9 md:text-lg" style={{borderColor:colors.primaryDark,color:"#173B2F"}}><CheckCircle2 size={21}/>Pogledaj kako radi</Button></a></div>
      ...
      <section id="opcije" className="px-5 py-20 md:px-8" style={{backgroundColor:colors.sageLight}}><div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Opcije" title="Sve važno na jednom mestu." text="Fokus je na stvarima koje roditelji najčešće prate tokom dana."/><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{features.map(([Icon,title,text])=><Card key={title} className="rounded-[28px] border bg-white shadow-sm" style={{borderColor:colors.border}}><CardContent className="p-7"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl" style={{backgroundColor:colors.cream,color:colors.primary}}><Icon size={24}/></div><h3 className="text-xl font-semibold" style={{color:colors.text}}>{title}</h3><p className="mt-3 leading-7" style={{color:colors.body}}>{text}</p></CardContent></Card>)}</div></div></section>
      <section id="kako-radi" className="overflow-hidden px-5 py-24 md:px-8" style={{backgroundColor:"#FFFDF8"}}><div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Kako radi" title="Jednostavno od prvog dana." text="Od prvog unosa do pregleda dana, sve je napravljeno da bude brzo, jasno i bez nepotrebnog razmišljanja."/><div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">{[{n:"01",t:"Napravite dnevnik",d:"Unesite ime bebe i datum rođenja. Možete odmah da krenete ili da probate demo dnevnik.",img:"/step-welcome.png",tilt:"left"},{n:"02",t:"Dodajte aktivnost",d:"Izaberite dojenje, pelenu, spavanje, dohranu ili drugu aktivnost i po potrebi promenite vreme.",img:"/step-add.png"},{n:"03",t:"Pratite dnevnik",d:"Sve aktivnosti se čuvaju po vremenu, pa lako vidite šta se desilo tokom dana.",img:"/step-dnevnik.png"},{n:"04",t:"Pogledajte pregled dana",d:"Brzo proverite ukupno dojenje, dohranu, spavanje, pelene i broj unosa za izabrani dan.",img:"/step-pregled.png",tilt:"right"}].map(step=><motion.div key={step.n} initial={{opacity:0,y:22}} whileInView={{opacity:1,y:0}} viewport={{once:true,margin:"-80px"}} transition={{duration:.55}} className="relative rounded-[36px] border bg-white p-5 shadow-[0_20px_70px_rgba(64,68,60,0.07)]" style={{borderColor:colors.border}}><div className="mb-5"><span className="rounded-full px-4 py-2 text-sm font-semibold" style={{backgroundColor:"#E7EFE0",color:"#5F8A52"}}>Korak {step.n}</span></div><PhoneMockup src={step.img} alt={step.t} tilt={step.tilt}/><div className="pt-7"><h3 className="headline text-2xl font-semibold leading-tight" style={{color:colors.text}}>{step.t}</h3><p className="mt-3 leading-7" style={{color:colors.body}}>{step.d}</p></div></motion.div>)}</div></div></section>
